...
<div class="accordion" id="accordionContainer">
  ...
  `;const s=document.getElementById("modalBackdrop");document.getElementById("bookModal");const c=document.getElementById("modalCloseBtn"),i=document.getElementById("quantity"),d=document.getElementById("increase"),a=document.getElementById("decrease"),E=document.getElementById("addToCart"),u=document.getElementById("buyForm");document.getElementById("message"),document.body.style.overflow="hidden",document.documentElement.style.overflow="hidden",new H("#accordionContainer",{duration:300,showMultiple:!1,openOnInit:[]});const C=P.querySelectorAll(".ac-trigger");C.forEach(l=>{l.addEventListener("click",()=>{setTimeout(()=>{C.forEach(b=>{const _=b.closest(".ac"),B=b.querySelector(".trigger-up"),q=b.querySelector(".trigger-down");_.classList.contains("is-active")?(B.classList.remove("hidden"),q.classList.add("hidden")):(B.classList.add("hidden"),q.classList.remove("hidden"))})},10)})}),c.addEventListener("click",w),s.addEventListener("click",l=>{l.target===s&&w()}),document.addEventListener("keydown",l=>{l.key==="Escape"&&w()},{once:!0}),d.addEventListener("click",()=>{i.value=parseInt(i.value)+1,S()}),a.addEventListener("click",()=>{const l=parseInt(i.value);l>1&&(i.value=l-1,S())}),E.addEventListener("click",()=>{const l=parseInt(i.value)||1;y.show({title:"У кошику: ",message:`Додано до кошика: ${l} книг(и)`,position:"topCenter"})}),u.addEventListener("submit",l=>{l.preventDefault(),y.show({title:"Покупка: ",message:"Дякуємо за покупку!",position:"topCenter"})});const V=document.getElementById("bookTotalPrice");function S(){const l=parseInt(i.value)||1,b=r*l;V.textContent=`Total: $${b.toFixed(2)}`}}function w(){document.getElementById("modalBackdrop").classList.add("hidden"),document.body.style.overflow="",document.documentElement.style.overflow=""}document.addEventListener("DOMContentLoaded",()=>{const t=document.getElementById("back-to-top");window.addEventListener("scroll",()=>{t.style.display=window.scrollY>300?"block":"none"}),t.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})})});document.addEventListener("DOMContentLoaded",()=>{const t=document.querySelector(".modal-overlay.register-modal"),o=document.querySelector(".register-modal-close"),n=document.querySelector(".register-form"),r=n.querySelectorAll("input, textarea"),e=document.querySelector(".modal-event-title");document.querySelectorAll(".events-button").forEach(i=>{i.addEventListener("click",()=>{const a=i.closest(".swiper-slide").querySelector(".events-name").textContent.trim();e.textContent=a,t.style.display="flex",requestAnimationFrame(()=>{t.classList.add("show")}),document.body.classList.add("body-no-scroll")})}),o.addEventListener("click",c);function c(){t.classList.remove("show"),t.classList.add("hide"),setTimeout(()=>{t.style.display="none",t.classList.remove("hide"),document.body.classList.remove("body-no-scroll")},300)}n.addEventListener("submit",i=>{i.preventDefault();let d=!0;if(r.forEach(a=>{const u=a.closest(".register-label").querySelector(".error-notification");a.classList.remove("error"),u.classList.remove("show"),a.value.trim()?a.type==="email"&&!a.value.match(/^[^@\s]+@[^@\s]+\.[^@\s]+$/)&&(a.classList.add("error"),u.textContent="Enter a valid email",u.classList.add("show"),d=!1):(a.classList.add("error"),a.name==="name"?u.textContent="Enter your name":a.name==="email"?u.textContent="Enter your email":a.name==="message"&&(u.textContent="Enter your message"),u.classList.add("show"),d=!1)}),d){const a=e.textContent;y.success({title:"Success",message:`You have successfully registered for ${a}`,position:"topRight",class:"custom-toast"}),n.reset(),c()}}),r.forEach(i=>{i.addEventListener("input",()=>{const a=i.closest(".register-label").querySelector(".error-notification");i.value.trim()&&(i.classList.remove("error"),a.classList.remove("show"))})})});
